// Chunks that need to be assembled from sub-pieces
const SPLIT_CHUNKS = {
...
async function fetchGitHub(name) {
  const resp = await fetch(GH_BASE + name, {
    redirect: 'follow',
    headers: { 'User-Agent': 'Mozilla/5.0 (compatible; CoreBorn/1.0)', 'Accept': 'application/octet-stream' },
  });
...
export default async (request, context) => {
  ...
  const CORS = { 'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/octet-stream', 'Cache-Control': 'public, max-age=31536000, immutable' };
  ...
  try {
    if (SPLIT_CHUNKS[name]) {
      ...
      let offset = 0;
      for (const buf of pieces) { merged.set(new Uint8Array(buf), offset); offset += buf.byteLength; }
      ...
      if (!resp.ok) return new Response('Failed to fetch chunk: ' + resp.status, { status: 502 });
